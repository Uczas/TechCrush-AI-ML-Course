...
# ============================================================================
# THREADED TTS MANAGER - Continuous speech support
# ============================================================================
class ThreadedTTSManager:
    def __init__(self):
        self.engine = None
    ...
    def _process_speech_queue(self):
        ...
        while self.running:
            try:
                ...
                if message and not self.muted:
                    try:
                        # Create a new engine instance for each speech to ensure it works
                        engine = pyttsx3.init()
                        ...
                        if self.voices and len(self.voices) > 0:
                            engine.setProperty('voice', self.voices[0].id)
    ...
    def speak(self, text):
        ...
        if self.available and not self.muted and text:
    ...
    def set_voice(self, voice_index):
        ...
        if self.available and 0 <= voice_index < len(self.voices):
            try:
...
if 'tts_manager' not in st.session_state:
    st.session_state.tts_manager = ThreadedTTSManager()
    ...
    def process_frame(self, frame, save_detection=False):
        ...
        if detected_obstacles:
            ...
            if filtered_obstacles:
                ...
                # This will now work continuously
...
# Show TTS status
if st.session_state.tts_manager.available:
    st.sidebar.success("✅ Speech engine ready!")
    
    # Voice selection
    if st.session_state.tts_manager.voices:
        voice_names = [v.name for v in st.session_state.tts_manager.voices]
        selected_voice = st.sidebar.selectbox("Select Voice", voice_names, index=0)
        if selected_voice:
            voice_idx = voice_names.index(selected_voice)
            st.session_state.tts_manager.set_voice(voice_idx)
    
    # Rate and volume
    voice_rate = st.sidebar.slider("Speech Rate", 100, 200, VOICE_RATE, 5)
    voice_volume = st.sidebar.slider("Volume", 0.0, 1.0, VOICE_VOLUME, 0.05)
    st.session_state.tts_manager.set_rate(voice_rate)
    st.session_state.tts_manager.set_volume(voice_volume)
    ...
    st.sidebar.error("⚠️ Speech feedback is disabled")
    st.sidebar.info("💡 Check terminal for details")
...
st.markdown("*Enhanced with voice customization, file upload, and data export*")
st.markdown("---")
...
# ============================================================================
# WEBCAM MODE - COMPLETE FIXED VERSION
# ============================================================================
